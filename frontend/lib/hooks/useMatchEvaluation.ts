...
export function useMatchEvaluation(matchId: string | null): UseMatchEvaluationReturn {
  ...
  const doFetch = useCallback(async (id: string) => {
    ...
    if (!token) { setEvalError('Bạn cần đăng nhập.'); setEvalState('error'); return; }
    setEvalState('loading'); setEvalError(null);
    try {
      const data = await fetchMatchEvaluation(id, token);
      setEvaluation(data); setEvalState('success');
    } catch (e: unknown) {
  ...
  useEffect(() => {
    if (!matchId) {
      setEvaluation(null); setEvalState('idle'); setEvalError(null);
      setGaps(null); setGapsState('idle');
      reqCacheRef.current = {}; evCacheRef.current = {};
      setReqCache({}); setEvCache({});
      return;
    }
    doFetch(matchId);
  }, [matchId, doFetch]);

  const refetchEvaluation = useCallback(() => { if (matchId) doFetch(matchId); }, [matchId, doFetch]);

  const fetchGaps = useCallback(async () => {
    if (!matchId) return;
    const token = getToken(); if (!token) return;
    setGapsState('loading');
    try { setGaps(await fetchMatchGaps(matchId, token)); setGapsState('success'); }
    catch { setGapsState('error'); }
  }, [matchId]);

  const fetchRequirements = useCallback(async (criterionId: string, page = 1) => {
  ...
  const fetchEvidence = useCallback(async (requirementId: string) => {
  ...
  return {
    evaluation, evaluationState, evaluationError, refetchEvaluation,
    gaps, gapsState, fetchGaps,
    fetchRequirements, requirementsCache,
    fetchEvidence, evidenceCache,
  };
}
