...
Run these commands from the `frontend` directory before committing:
...
`format` applies the project's Prettier rules. `format:check` verifies the same rules without modifying files, so it is suitable for CI.
