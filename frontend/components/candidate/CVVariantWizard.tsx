...
export default function CVVariantWizard() {
  ...
  const runValidation = async () => {
    ...
    try {
      ...
      setActive((current) =>
        current ? { ...current, status: report.status, validator_result: report } : current,
      );
      setMessage(
        report.passed
          ? 'Đã vượt qua đủ 7 hard validators.'
          : 'Publish đang bị chặn; xem lỗi theo từng validator.',
      );
    } catch (validationError) {
      setError(
        validationError instanceof Error ? validationError.message : 'Không chạy được validator.',
      );
    } finally {
      setBusy('');
    }
    ...
    <section className="cv-variant-wizard" aria-labelledby="cv-variant-title">
      ...
      <div className="cv-variant-create-grid">
        {mode === 'HAS_CV' && (
          <label>
            CV nguồn
            <select value={cvId} onChange={(event) => setCvId(event.target.value)}>
        ...
        <div className="cv-variant-guided-form">
          ...
          <label className="wide">
            ...
            <textarea
              ...
              onChange={(event) => setContent((item) => ({ ...item, summary: event.target.value }))}
            />
          </label>
          <label className="wide">
          ...
          <label>
            Kinh nghiệm, mỗi dòng một mục
            <textarea
              value={recordsToText(content.experience)}
              onChange={(event) =>
                setContent((item) => ({ ...item, experience: textToRecords(event.target.value) }))
              }
            />
          </label>
          <label>
            Dự án, mỗi dòng một mục
            <textarea
              value={recordsToText(content.projects)}
              onChange={(event) =>
                setContent((item) => ({ ...item, projects: textToRecords(event.target.value) }))
              }
            />
          </label>
          <label>
            Học vấn, mỗi dòng một mục
            <textarea
              value={recordsToText(content.education)}
              onChange={(event) =>
                setContent((item) => ({ ...item, education: textToRecords(event.target.value) }))
              }
            />
          </label>
          <label className="cv-variant-confirm wide">
            ...
            />{' '}
            Tôi xác nhận toàn bộ thông tin trên là sự thật và cho phép tạo Candidate Evidence
            snapshot bất biến.
          </label>
        </div>
      )}
      ...
      <button
        className="cv-variant-primary"
        type="button"
        disabled={!canCreate || Boolean(busy)}
        onClick={() => void create()}
      >
        <Sparkles size={16} /> {busy === 'create' ? 'Đang tạo…' : 'Tạo CV Variant'}
      </button>
      ...
      {active && (
        <div className="cv-variant-editor">
          <div className="cv-variant-status-row">
            ...
            <small>
              Revision {active.revision_no} ·{' '}
              {busy === 'autosave' ? 'Đang autosave…' : dirty ? 'Chưa lưu' : 'Đã lưu'}
            </small>
          </div>
          <div className="cv-variant-score-row">
            <span>
              Match trước: <b>{content._match_scores?.before ?? 0}%</b>
            </span>
            <span>
              Preview sau tối ưu: <b>{content._match_scores?.after_preview ?? 0}%</b>
            </span>
            <span>
              AI: <b>{active.ai_metadata.provider || 'fallback'}</b>
            </span>
          </div>

          <section>
            <h4>Chỉnh nội dung theo từng section</h4>
          ...
          {active.status !== 'PUBLISHED' && (
            <section className="cv-variant-writeback">
              <h4>Xác nhận fact mới do bạn tự nhập</h4>
              <p>
                AI không thể xác nhận thay bạn. Dán đúng một câu mới và đánh dấu xác nhận trước
                autosave.
              </p>
              <textarea
                value={confirmedClaim}
                onChange={(event) => setConfirmedClaim(event.target.value)}
                placeholder="Ví dụ: Đạt chứng chỉ AWS Cloud Practitioner tháng 6/2026"
              />
              <label>
                <input
                  type="checkbox"
                  checked={confirmClaimChecked}
                  onChange={(event) => setConfirmClaimChecked(event.target.checked)}
                />{' '}
                Tôi xác nhận câu này là sự thật.
              </label>
            </section>
          )}

          <section>
            <h4>Đề xuất AI — review từng thay đổi</h4>
            {suggestions.length ? (
              <div className="cv-variant-suggestions">
                {suggestions.map((suggestion) => (
                  <article key={suggestion.id}>
                    <header>
                      <strong>{suggestion.section || 'Nội dung CV'}</strong>
                      <span>{suggestion.validator_status}</span>
                    </header>
                    <div className="cv-variant-diff">
                      ...
                      <label>
                        <b>Đề xuất / chỉnh sửa</b>
                        <textarea
                          disabled={
                            active.status === 'PUBLISHED' || suggestion.decision === 'reject'
                          }
                          value={
                    ...
                    <p>
                      <b>Lý do:</b> {suggestion.reason}
                    </p>
                    <p>
                      <b>Evidence:</b>{' '}
                      {suggestion.source_spans?.map((span) => span.text).join(' · ') || 'Không có'}
                    </p>
              ...
              <p>Không có rewrite nào đủ evidence để đề xuất.</p>
            )}
          </section>
          ...
          <section className="cv-variant-validation">
            <div className="cv-variant-validation-head">
              ...
              <button
                ...
                disabled={Boolean(busy) || dirty || active.status === 'PUBLISHED'}
              >
                <FileCheck2 size={16} /> Chạy 7 validators
              </button>
            </div>
            {validation && (
              <>
                ...
                <p>
                  {validation.claims_supported}/{validation.claims_total} claims có evidence · PDF{' '}
                  {validation.render.pages} trang
                </p>
              </>
            )}
            <div className="cv-variant-publish-actions">
              <button
                type="button"
                onClick={() => void openPdf(true)}
                disabled={!validation?.passed || Boolean(busy)}
              >
              ...
              <button
                ...
                disabled={!validation?.passed || Boolean(busy) || active.status === 'PUBLISHED'}
              >
                <Save size={16} /> Publish phiên bản
              </button>
              {active.status === 'PUBLISHED' && (
                <button type="button" onClick={() => void openPdf(false)} disabled={Boolean(busy)}>
          ...
          <section>
            ...
            <ol className="cv-variant-revisions">
              {active.revisions.map((revision) => (
                <li key={revision.revision_no}>
                  <strong>Revision {revision.revision_no}</strong>
                  <span>
                    {revision.editor_type} · {revision.change_summary}
                  </span>
                </li>
              ))}
            </ol>
          ...
          variants.map((variant) => (
            <button type="button" key={variant.id} onClick={() => void openVariant(variant.id)}>
              <span>
                ...
                <small>
                  {statusLabel(variant.status)} · revision {variant.revision_no}
                </small>
              </span>
              <span>Mở →</span>
            </button>
