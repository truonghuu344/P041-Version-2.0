...
const steps = [
  {
    ...
    title: 'Match CV',
    content: 'Chọn CV và công việc để xem mức độ phù hợp và gợi ý cải thiện.',
  },
  {
    targetId: 'intent-card-cv',
    title: 'Tạo hoặc cập nhật CV',
    content: 'Bắt đầu từ mẫu có sẵn hoặc tải CV hiện có lên hệ thống.',
  },
  {
    targetId: 'intent-card-interview',
    title: 'Luyện phỏng vấn',
    content: 'Thực hành trả lời bằng giọng nói và nhận phản hồi theo phương pháp STAR.',
  },
];
    ...
    const update = () => {
      ...
      setPosition({
        top: Math.min(
          ...
          Math.max(12, isCompact ? rect.top + 12 : rect.top + 18),
        ),
        left: Math.min(window.innerWidth - 310, Math.max(12, isCompact ? 12 : rect.right + 16)),
      });
    };
    update();
  ...
  return (
    <aside className="buddy-tour" style={position} aria-live="polite">
      <div className="buddy-tour-mascot">
        <Image src="/images/buddy1.png" alt="Career Buddy" width={44} height={44} />
      </div>
      <div className="buddy-tour-card">
        <button
          type="button"
          className="buddy-tour-close"
          onClick={close}
        ...
        <footer>
          ...
          >
            {step === steps.length - 1 ? 'Hoàn thành' : 'Tiếp tục'} <ChevronRight size={14} />
          </button>
        </footer>
      </div>
    </aside>
